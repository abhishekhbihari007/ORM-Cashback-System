    ...
    <section className="relative overflow-hidden min-h-[600px] bg-white py-20 flex items-center">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 grid gap-8 lg:gap-12 grid-cols-1 lg:grid-cols-12 lg:items-center relative z-10 w-full">
        <div className="space-y-4 lg:col-span-7 relative z-10 text-left">
          ...
          <p className="text-xl text-slate-700 max-w-2xl md:text-2xl mt-4 mb-6 text-left">
          ...
          <div className="flex flex-wrap gap-3 relative z-10 mb-8">
            <Link
              href="/signup"
              className="rounded-full bg-gradient-to-r from-orange-500 to-red-600 px-8 py-4 text-lg text-white shadow-lg shadow-orange-500/25 font-semibold hover:from-orange-600 hover:to-red-700 transition"
            >
              Sign Up
            </Link>
            <Link 
              href="/login" 
              className="rounded-full bg-white border border-slate-300 text-slate-700 px-8 py-4 text-lg font-semibold hover:bg-slate-50 transition relative z-10 cursor-pointer inline-block"
            >
              Login
            </Link>
          </div>
          <div className="flex flex-wrap gap-4">
            {heroMetrics.map((metric) => (
              <div key={metric.label} className="min-w-[150px] flex-1 md:flex-none md:w-auto">
                <StatCard {...metric} />
              </div>
            ))}
          </div>
        </div>
        <div className="relative flex justify-center items-center lg:col-span-5 pt-8 lg:pt-0">
          <StarRatingGraphCard />
        </div>
      </div>
      ...
      <div className="absolute bottom-0 right-0 z-20 pointer-events-none overflow-hidden">
        <div className="relative" style={{ width: '900px', height: '450px' }}>
          <Image 
